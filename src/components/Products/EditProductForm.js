...
const EditProductFrom = ({
  ...
  const [inventory, setInventory] = useState(
    product === undefined ? '' : product.inventory
  );
  const [price, setPrice] = useState(
    product === undefined ? '' : product.currentPrice
  );
  const [sale, setSale] = useState(product === undefined ? '' : product.sale);
  const [category, setCategory] = useState(
    product === undefined ? '' : product.categories[0]
  );
  ...
  async function handleSubmit(event) {
    ...
    const removeIndex = products.findIndex(
  ...
  return (
    <div className='editProductContainer'>
      ...
      <form className='editProductForm' onSubmit={handleSubmit}>
        ...
        <input id='editProductSubmit' type='submit' value='Submit'></input>
      </form>
    </div>
  );
};
